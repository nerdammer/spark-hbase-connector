...
class HBaseSparkConfTest extends FlatSpec with Matchers {
  ...
  "A default HBaseSparkConf object" should "have a host property set to localhost without other configurations" in {
    ...
    val hadoopConf = HBaseSparkConf.fromSparkConf(new SparkConf).createHadoopBaseConfig()
    hadoopConf.get(HConstants.ZOOKEEPER_QUORUM) should be (HBaseSparkConf.DefaultHBaseHost)
  }
